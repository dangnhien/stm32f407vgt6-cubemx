...
uint8_t btn_current, btn_last = 0;
...
void btl_press_short_callback()
{
	test=3;
}

void btl_press_slow_callback()
{
	test=4;
}
...
{
	// -----------------Loc nhieu----------------------
	uint8_t status_btn = HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0);
	
	if(status_btn != btn_filter)
	{
		btn_filter = status_btn;
		is_debouncing = 1;
	...
	// ---------------------- Xu ly nhan giu-------------------------------
	if(press_slow == 1  && HAL_GetTick()-time_start_press >= 3000)
	{
		press_slow = 0;	// do for function btl_press_slow_callback(); only run once
